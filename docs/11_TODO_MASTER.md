...
- [x] Tienda de electrodomésticos y tecnología (`appliance_store`): soporte modular completo para ciclo de vida de números de serie (`serial_numbers`) y pólizas de garantía (`warranty`). Tabla `product_serials` multi-tenant aislada con estados `available`, `reserved`, `sold`, `returned`, `defective`. Bloqueo pesimista de concurrencia contra venta de seriales duplicados o ya vendidos. Flujo híbrido con series pre-cargadas en almacenes (con autocompletado en tiempo real en POS) y captura flexible al vuelo con registro automático. Reversión a disponible en caso de anulación de factura. Pestaña de Series y Garantías en Inventario con carga masiva de lotes y buscador de trazabilidad. Empresa demo **ElectroHogar Dominicana** provisionada con 2 almacenes, 90 seriales precargados, secuencias B01/B02, catálogo con garantías y suites de prueba `ApplianceStoreTest` y `ProductSerialTrackingTest` (100% verdes).
