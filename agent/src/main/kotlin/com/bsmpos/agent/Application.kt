...
private val logger = LoggerFactory.getLogger("BSMPOSAgent")
...
data class PrinterTestRequest(
    ...
    val message: String = "BSM-POS Windows Agent - Prueba de impresora",
)
...
fun Application.agentModule(config: AgentConfig, printerService: LocalPrinterService = LocalPrinterService()) {
    ...
    routing {
        ...
        get("/api/status") {
            ...
            call.respond(AgentStatusResponse("ok", "BSM-POS Windows Agent", config.version, config.host, config.port))
        }
        get("/api/config") {
            if (!call.enforce(security)) return@get
            call.respond(
    ...
    logger.info("BSM-POS Windows Agent {} escuchando en {}:{}", config.version, config.host, config.port)
}
