...
/** Configuración de ejecución para el agente de hardware local de BSM-POS. */
data class AgentConfig(
    ...
    val terminalId: String = "BSMPOS-TERMINAL",
) {
    init {
    ...
    companion object {
        fun from(config: ApplicationConfig): AgentConfig = AgentConfig(
            ...
            terminalId = config.propertyOrNull("agent.terminal_id")?.getString()?.trim()?.ifBlank { "BSMPOS-TERMINAL" }
                ?: "BSMPOS-TERMINAL",
        )
